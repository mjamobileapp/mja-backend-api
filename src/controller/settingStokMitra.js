...
const createNewSetting = async (req, res) => {
  ...
  try {
    ...
    if (["Mitra tidak ditemukan atau tidak aktif", "Item tidak ditemukan atau tidak aktif", "Setting stok untuk item ini sudah ada pada mitra tersebut"].includes(error.message)) {
...
const updateSetting = async (req, res) => {
  ...
  if (!body.stokMinimum || !body.updatedBy) {
...
const getAllSettings = async (req, res) => {
  ...
  try {
    const data = await SettingStokModel.getAllSettings(status);
...
const deleteSetting = async (req, res) => {
  ...
  try {
    await SettingStokModel.deleteSetting(id, username);
    res.status(200).json({ message: "Delete success", data: null });
  } catch (error) {
    if (error.message === "data not found") return res.status(404).json({ error: error.message });
    res.status(500).json({ message: "Server Error", serverMessage: error.message });
  }
...
const restoreSetting = async (req, res) => {
  const { id } = req.params;
  const username = req.user.username;
  try {
    await SettingStokModel.restoreSetting(id, username);
    res.status(200).json({ message: "Restore success", data: null });
  } catch (error) {
    if (error.message === "data not found") return res.status(404).json({ error: error.message });
    res.status(500).json({ message: "Server Error", serverMessage: error.message });
  }
